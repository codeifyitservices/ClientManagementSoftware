...
export default function ServiceSettingsPage({ token, showToast, onRestoreSuccess }) {
  ...
  const API_SERVICES = `${import.meta.env.VITE_BACKEND_URL}/api/services`;
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errData.message || "Failed to save service configuration.",
        );
      }
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-semibold mt-1">
              Configure prefilled billing services, SAC tax codes, and GST
              rates.
            </p>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700 font-semibold">
                {services.length === 0 ? (
                  <tr>
                    ...
                    >
                      No billing services configured. Click Add Service to
                      register one.
                    </td>
                  </tr>
                ) : (
